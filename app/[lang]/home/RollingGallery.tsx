...
const RollingGallery: React.FC<RollingGalleryProps> = ({ autoplay = false, pauseOnHover = false, images = [] }) => {
  ...
  const cylinderWidth: number = isScreenSizeSm ? 1500 : 1500;
          ...
          {galleryImages.map((url, i) => (
            ...
            >
              <img
                src={url}
                alt="gallery"
                className="pointer-events-none h-[400px] w-[400px] rounded-[15px] border-[3px] border-white object-cover transition-transform duration-300 ease-out group-hover:scale-105 sm:h-[100px] sm:w-[220px]"
              />
            </div>
          ))}
        </motion.div>
